...
- 探す（`/`）: 検索用見出し・フリーワード・時間帯・予算を常時表示。都県/エリア/ジャンルはPC左サイドバー、モバイルはチップ横列。件数・並び替えの下に比較用の縦型店舗一覧。詳細条件ドロワーも維持。条件変更は即時反映。都県切替時は他条件を初期化、リセットは都県を維持。2026-09-08のUI刷新設計を参照。
...
- ナビ: 全画面の共通ヘッダーにブランド・日英切替、PCでは探す/保存リンク。モバイルは下部2タブ（探す `/`・保存 `/saved`）でセーフエリアを避ける。
