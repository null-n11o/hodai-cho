...
**Architecture:** 同梱の透過PNGを共通SiteHeaderから使用する。既存の言語別ホームリンクとナビゲーションを維持する。SSGのHTMLでも同じ画像が参照される。
...
**Interfaces:** controller supplies transparent assets; SiteHeader consumes `/brand/tabeho-logo-header.png`; favicon consumes `/brand/tabeho-symbol.png`. Set HTML image width/height from actual dimensions after assets arrive, preserving aspect ratio.
...
- [x] Step 4: GREEN: run focused tests, `npm test`, `npm run lint`, `npm run build:ssg`. Controller visually checks mobile/desktop JA/EN and asset transparency. Fix any overflow or fuzzy/mismatched asset before handoff.
